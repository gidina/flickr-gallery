...
class App extends Component {
    state = { isLoading: true, fotosGaleria: null, fotoSeleccionada: null, isLoadingModal: false }
    componentDidMount = () => {
        fetch(API_URL_GET_PHOTOS)
        .then(response => response.json())
        .then(json => this.setState({ ...this.state, isLoading: false, fotosGaleria: json.photos.photo }));
    }
    onClickCloseModal = () => {
    ...
    onClickPhotoItemHandler = (photo) => {
        ...
        this.setState({ ...this.state, isLoadingModal: true, fotoSeleccionada: {
            title: photo.title,
    ...
    render = () => {
        const { isLoading, fotosGaleria, fotoSeleccionada, isLoadingModal } = this.state;
        return (
            <Fragment>
                {
                    isLoading ?
                    ...
                    <Fragment>
                        <Gallery photos={fotosGaleria} onClickPhoto={this.onClickPhotoItemHandler}/>
                        { 
                            fotoSeleccionada ? 
                            <Modal isLoading={isLoadingModal} photo={fotoSeleccionada} onClose={this.onClickCloseModal} /> : 
                            null 
                        }
                    </Fragment>
                }
